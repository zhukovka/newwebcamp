...
function mcImgSlider(k) {
    ...
    var j = new A,
        ...
        ib = function (b) {
            var a = x[d];
            if (a)
                while (a--) x[a][l] = a != b && x[a].on == 0 ? "thumb" : "thumb thumb-on"
        }, tb = function () {
            var f;
            if (a.l) f = Q(a.l);
            if (f)
                for (var g = f.childNodes, e = 0; e < g[d]; e++) g[e][l] == "thumb" && x.push(g[e]);
            var c = x[d];
            if (c) {
                while (c--) {
                    x[c].on = 0;
    ...
    lb.prototype = {
        c: function () {
            ...
            else b.e = db(g[b.a], "img")[0]; if (g[b.a][w] == "A") g[b.a][c][N] = "block";
            e[c][P] = 'url("' + b.e[m]("src") + '")';
            R = this.k();
            var h = b.e[u];
        ...
        g: function () {
            ...
            while (a--) {
                ...
                else d[a][l] = ""; if (g[a][w] == "A") g[a][c][N] = a == b.a ? "block" : "none"
            }
        },
        i: function () {
            M = J("mc-caption");
        ...
        x: function () {
            for (var k = [], j = Math.round(t / a.g), i = Math.round(z / a.m), g = 0; g < a.m; g++)
                for (var f = 0; f < a.g; f++) {
                    var d = J("mcBox");
                    d[c].left = j * f + "px";
                    d[c][X] = i * g + "px";
    ...
    var xb = function () {
        ...
        fb = function (c) {
            if (++mb < 20)
                if (!n || typeof tooltip == r) E(function () {
                    fb(c)
                }, 300);
                else
                    for (var b = D(O), a = 0; a < b[d]; a++) b[a].onmouseover = function () {
